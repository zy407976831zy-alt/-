...
2. 禁止把客户变成参考图里的人，禁止参考图模特脸，禁止通用 AI 美女脸，禁止网红脸，禁止重新设计五官。
3. 女士和男士都必须分别像原片本人；多人合照中不能只保留一个人，不能让另一人跑脸。
4. 眼型、眼距、眉眼关系、鼻梁、鼻头、鼻翼、人中、嘴型、嘴角、脸宽、脸长、下颌线、颧骨、太阳穴、面部痣点、年龄感和真实气质必须贴近原片。
5. 参考图只允许提供风格、光影、色彩、背景氛围、服装语言、构图方向和后期质感；绝对不能提供人物身份、脸型、五官、表情模板、年龄感或气质模板。
...
以下是风格生成任务说明。所有说明都必须服从上方“人物身份绝对锁定”规则；任何会导致人物不像客户原片的要求都自动失效。

${fields.prompt || ""}

【最终红线复核】
生成完成前再次确认：客户原片人物身份必须绝对保留。参考图人物不能迁移，脸不能变，五官不能重绘，年龄感不能变，气质不能变。`;
  }
  return `${identityFirewall}

这是“只精修原片”任务，不是重新生成写真。
...
【最终红线复核】
生成完成前再次确认：客户原片人物身份必须绝对保留。脸不能变，五官不能重绘，年龄感不能变，动作和表情不能变。`;
}
