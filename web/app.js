...
function applyProcessModeRules() {
  ...
  for (const id of ["hairOption", "backgroundOption", "wardrobeOption", "poseOption", "bodyRetouchOption", "sourceCorrectionOption", "retouchOption", "colorGradeOption"]) {
...
- 参考图重点优先级高于通用风格描述；如果通用风格和参考图重点冲突，以参考图重点为准。
...
- 如果发型参考为 \`匹配参考\`：这是强约束。发型必须明显接近参考图，包括头发长度视觉、分缝位置、发量、头顶高度、贴头或蓬松程度、卷直程度、发尾方向、脸侧发丝、额前碎发、盘发/披发结构、头纱/头饰位置和整体轮廓；但必须保留客户发际线合理性、脸型、头型、年龄感和真实身份。
...
- 如果服装参考为 \`匹配参考\`：这是强约束。必须优先按参考图可见服装结构生成，尽量接近参考图的颜色、材质、领口、袖型、腰线、裙摆、头纱、饰品、褶皱层次和整体穿搭轮廓；如果参考图头纱是视觉重点，必须生成明显头纱并保留其形态、面积、透明纱质和视觉存在感；不能只保留原片服装不变；但必须保持真实人体结构、合理遮挡、自然布料逻辑和客户身体比例。
...
- 如果动作表情为 \`重新设计\`：这是强约束。必须根据参考图的人物姿态、身体朝向、手臂位置、头部角度、视线方向、情绪表情和头纱/裙摆/花束的动态关系重新设计动作；不能继续保留原片动作不变；如果参考图的头纱展示依赖某个动作、角度或构图，必须围绕这个头纱展示重新安排姿态；但必须保留客户脸、年龄感、身体比例和真实身份。
